...
if __name__ == "__main__":
    ...
    input_folder = f'{root_path}'
    output_folder = f'results/initial_cleanup/'
    sample_map = 'raw_data/labels_peptides.xlsx'
    
    # Read in sample map
    sample_map = pd.read_excel(sample_map, sheet_name='channel-directory')  
    sample_map['Channel #'] = sample_map['Channel #'].astype(str) 

    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    tmt_cleaner(input_folder, output_folder, sample_names=['Total-Total', 'HT7-Beads', 'HT7-Supernatant', 'AT8-Beads', 'AT8-Supernatant', 'T181-Beads', 'T181-Supernatant', 'MC1-Beads', 'MC1-Supernatant', 'TOMA-Beads', 'TOMA-Supernatant', ], proteins_file='proteinGroups.txt', peptides_file='peptides.txt', prot_cols=[], pep_cols=['Missed cleavages'])
    
    # Generate compiled version
    compiled = []
    for proteins in os.listdir(output_folder):
        if 'Proteins.csv' not in proteins:
            continue
        proteins_df = pd.read_csv(f'{output_folder}{proteins}')
        proteins_df.drop([col for col in proteins_df.columns.tolist() if 'Unnamed: ' in col], axis=1, inplace=True)
        # Collect columns of interest
        group_cols = ['Protein IDs', 'Gene names', 'Protein names', 'Number of proteins']
        value_cols = [col for col in proteins_df.columns.tolist() if 'Reporter intensity corrected ' in col]
        # Melt into longform data
        proteins_df = proteins_df[group_cols + value_cols].copy()
        proteins_df = pd.melt(
            proteins_df,
            id_vars=group_cols,
        ...
        proteins_df['channel'] = proteins_df['channel'].str.split(' ').str[3]
        # Add channel info
        proteins_df['sample'] = proteins_df['channel'].map(dict(sample_map[['Channel #', 'Sample']].values))
        proteins_df[['disease', 'patient']] = proteins_df['sample'].str.split('-', expand=True)
        
        proteins_df['pulldown'] = proteins.split('_')[0]
        proteins_df[['antibody', 'pulldown-type']] = proteins_df['pulldown'].str.split('-', expand=True)
        
        compiled.append(proteins_df)
    
    compiled = pd.concat(compiled)
    
    compiled.to_csv(f'{output_folder}Compiled_Proteins.csv')
